...
- **Validation + Refinement layer** — after generation and cross-file coherence, a symbol index is built across all generated files; undefined component references, bad runtime imports (`react-router-dom` hooks outside a Router, missing `<BrowserRouter>`), and broken `<Route element={<X />}>` targets are detected; affected files go through up to 2 targeted LLM correction passes; duplicate import lines are removed automatically
- **Intent routing system** — three-stage classification pipeline: (1) hard regex pre-check for project-creation messages forces `PROJECT_BUILDER` before any LLM call; (2) LLM classifier with structured JSON response and confidence threshold, 12 s timeout with automatic 20 s retry on cold-start delays; (3) smart heuristic fallback that detects creation/multi-file/edit intent from message content instead of naively defaulting to chat. Context assembly is now intent-aware: Builder runs with a clean prompt (no current file, selection, pinned files, or project tree); Chat receives no project context unless the user attaches a file or the message explicitly references local code.

**Fix**
- `node_modules` deletion failing on Windows when esbuild.exe or other binaries are marked read-only (`EACCES`) — backend now strips read-only bits via `filepath.Walk` + `os.Chmod(0777)` before `os.RemoveAll`
- Multi-delete via right-click context menu deleted items one at a time — Delete button now checks the full selection and routes to `doMultiDelete` when multiple items are selected
